        ...
        viewModel.viewEvents.observe(viewLifecycleOwner) {
            when (it) {
                ...
                is PlaygroundViewEvents.OpenScratch -> {
                    //TODO: change here
                    navigator.launchScratchApp(requireActivity(), Bundle())
                    //val intent = Intent(requireContext(), ScratchActivity::class.java)
                    //startActivity(intent)
                }
                is PlaygroundViewEvents.OpenScratchWithFile -> {
                    navigator.launchScratchApp(requireActivity(), Bundle().apply { putSerializable(Constants.INTENT_EXTRA_KEY_FILE,it.file) })
                    /*val intent = Intent(requireContext(), ScratchActivity::class.java)
