...
class MerakiNavigator(
    ...
    private val scratchAppModuleNavigator: ScratchAppModuleNavigator? by lazy {
        val serviceIterator = ServiceLoader.load(
            ScratchAppModuleNavigator::class.java,
            ScratchAppModuleNavigator::class.java.classLoader
        ).iterator()
        if (serviceIterator.hasNext()) {
            serviceIterator.next()
        } else {
    ...
    fun launchScratchApp(activity: FragmentActivity, bundle: Bundle) {
        if (dynamicFeatureModuleManager.isInstalled(SCRATCH_MODULE_NAME)) {
            scratchAppModuleNavigator?.launchScratchApp(activity, bundle)
        } else {
            val progress = ProgressDialog(activity).apply {
                setCancelable(false)
                setMessage(activity.getString(R.string.installing_module_message))
